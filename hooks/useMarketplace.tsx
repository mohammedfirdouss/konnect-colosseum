...
export const useMarketplace = () => {
  ...
  const getMarketplaceAddress = useCallback(
    (authority: PublicKey | null): PublicKey | null => {
  ...
  // Use existing marketplace PDA as default
  const DEFAULT_MARKETPLACE = new PublicKey(
    process.env.NEXT_PUBLIC_MARKETPLACE_ADDRESS ||
      "p9vMGZkbh6psLoAwNkHU5qJkSvqL2zETBgoVJRqTvno"
  );

  // Get marketplace address - use existing marketplace instead of generating new one
  const marketplaceAddress = DEFAULT_MARKETPLACE;
  ...
  const fetchAllListings = useCallback(async () => {
    ...
    try {
      ...
      // Use the default marketplace address
  ...
  const initMarketplace = useCallback(
    async (feeBps: number) => {
      try {
        ...
        const marketplaceService = new MarketplaceService(program, programId);
        const tx = await marketplaceService.initMarketplace(publicKey, feeBps);
  ...
  const registerMerchant = useCallback(
    async (marketplace: PublicKey, owner: PublicKey) => {
      try {
        if (!connected || !publicKey) {
          throw new Error("Wallet not connected");
        }
        ...
        const marketplaceService = new MarketplaceService(program, programId);
        const tx = await marketplaceService.registerMerchant(
          marketplace,
          owner
        );
  ...
  const getListingsBySeller = useCallback(
    async (marketplace: PublicKey, seller: PublicKey) => {
      try {
        if (!connected || !publicKey) {
          throw new Error("Wallet not connected");
        }
        ...
        const marketplaceService = new MarketplaceService(program, programId);
        return await marketplaceService.getListingsBySeller(
          marketplace,
          seller
        );
      } catch (error) {
  ...
  const createGoodsOrderWithEscrow = useCallback(
    async (
      marketplace: PublicKey,
      listing: PublicKey,
      buyerAta: PublicKey,
      ...
      try {
        ...
        const marketplaceService = new MarketplaceService(program, programId);
        const tx = await marketplaceService.createGoodsOrderWithEscrow(
          marketplace,
          listing,
          publicKey,
          buyerAta,
        ...
        toast.success("Order created! Funds are in escrow.");
        return tx;
      } catch (error: any) {
        console.error("Error creating goods order:", error);
  ...
  const releaseServiceOrder = useCallback(
    async (
      escrow: PublicKey,
      marketplace: PublicKey,
      listing: PublicKey,
  ...
  // Get buyer's ongoing orders
  const getBuyerOngoingOrders = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const marketplaceService = new MarketplaceService(program, programId);
      return await marketplaceService.getBuyerOngoingOrders(publicKey);
    } catch (error: any) {
  ...
  const getBuyerCompletedOrders = useCallback(async () => {
    if (!program || !publicKey || !connected) {
      throw new Error("Wallet not connected or program not initialized");
    }
    ...
    try {
      setLoading(true);
      const marketplaceService = new MarketplaceService(program, programId);
      return await marketplaceService.getBuyerCompletedOrders(publicKey);
    } catch (error: any) {
  ...
  const getSellerPendingOrders = useCallback(async () => {
    if (!program || !publicKey || !connected) {
      throw new Error("Wallet not connected or program not initialized");
    }
    ...
    try {
      setLoading(true);
      const marketplaceService = new MarketplaceService(program, programId);
      return await marketplaceService.getSellerPendingOrders(publicKey);
    } catch (error: any) {
  ...
  // Get seller's ongoing orders
  const getSellerOngoingOrders = useCallback(async () => {
    if (!program || !publicKey || !connected) {
      throw new Error("Wallet not connected or program not initialized");
    }
    ...
    try {
      setLoading(true);
      const marketplaceService = new MarketplaceService(program, programId);
      return await marketplaceService.getSellerOngoingOrders(publicKey);
    } catch (error: any) {
  ...
  const getSellerCompletedOrders = useCallback(async () => {
    if (!program || !publicKey || !connected) {
      throw new Error("Wallet not connected or program not initialized");
    }
    ...
    try {
      setLoading(true);
      const marketplaceService = new MarketplaceService(program, programId);
      return await marketplaceService.getSellerCompletedOrders(publicKey);
    } catch (error: any) {
  ...
  return {
    ...
    createGoodsOrderWithEscrow, // Create goods order with escrow
    ...
    getMarketplace,
    getAllMerchants,
    getBuyerOngoingOrders, // Get buyer's ongoing orders
    getBuyerCompletedOrders, // Get buyer's completed orders
    getSellerPendingOrders, // Get seller's pending orders
    getSellerOngoingOrders, // Get seller's ongoing orders
    getSellerCompletedOrders, // Get seller's completed orders
